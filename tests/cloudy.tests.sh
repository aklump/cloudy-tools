...
function testConfigurationMerge() {
  ...
  eval $(get_config_as "zebra" -a "tests.config.associative_merge_test.z")
  assert_same "zebra" "$zebra"

  eval $(get_config_as "yak" -a "tests.config.associative_merge_test.y")
  assert_same "yak" "$yak"

  eval $(get_config_as "xylitol" -a "tests.config.associative_merge_test.x")
  assert_same "xylitol" "$xylitol"

  eval $(get_config_as "whisky" -a "tests.config.associative_merge_test.w")
  assert_same "whisky" "$whisky"
...
function testFunctionExists() {
  function_exists bogus
  assert_exit_status 1

  function not_so_bogus() {
    return 0
  }
  function_exists not_so_bogus
  assert_exit_status 0
...
function testTempdirUsesExistingDirectoryPerArgument() {
  local dir=$(tempdir "com.apple.aklump.cloudy")
  assert_exit_status 0
  ...
  local dir2=$(tempdir "com.apple.aklump.cloudy")
  assert_exit_status 0
...
function testEventListenAndDispatchWithOnEventExplicitFunctionName() {
  ...
  event_dispatch "test_bravo" "do re" "mi"
  assert_exit_status 0
...
function testEventListenAndDispatchWithCustomFunctionName() {
  ...
  event_dispatch "test_alpha" "do re" "mi"
  assert_exit_status 0
...
function testArrayHasValue() {
  ...
  array_has_value 'value1'
  assert_exit_status 0
  assert_same 0 $array_has_value__index

  array_has_value 'no_value'
  assert_exit_status 1
}
...
function testValidateCommandWorksForMasterCommandsAndAliases() {
  _cloudy_validate_command "clear-cache"
  assert_exit_status 0
  _cloudy_validate_command "clearcache"
  assert_exit_status 0
  _cloudy_validate_command "cc"
  assert_exit_status 0
}
